...
import co.sinte.entities.Listas.riesgos.COAGYArea;
...
public class COAGYAreaDAO {
    ...
    COAGYArea sreturn = null;

    public ArrayList COAGYArea(ArrayList coagyarea, MensajeRespuesta respuesta) {
        ...
        if (con != null) {
            try {
                ...
                while (rs.next()) {
                    ...
                    sreturn = new COAGYArea();

                    sreturn.setId(rs.getLong(1));
                    sreturn.setValor(rs.getString(2));

                    coagyarea.add(sreturn);

                }

            } catch (SQLException ex) {
                Logger.getLogger(COAGYAreaDAO.class.getName()).log(Level.SEVERE, null, ex);
