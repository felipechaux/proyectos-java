...
import co.sinte.entities.Listas.riesgos.COAGYBase;
...
public class COAGYBaseDAO {
    ...
    COAGYBase sreturn = null;

    public ArrayList COAGYBase(ArrayList coagybase, MensajeRespuesta respuesta) {
        ...
        if (con != null) {
            try {
                ...
                while (rs.next()) {
                    ...
                    sreturn = new COAGYBase();

                    sreturn.setId(rs.getLong(1));
                    sreturn.setValor(rs.getString(2));

                    coagybase.add(sreturn);

                }

            } catch (SQLException ex) {
                Logger.getLogger(COAGYBaseDAO.class.getName()).log(Level.SEVERE, null, ex);
